...
def test_logs_from_file(monkeypatch, tmp_path):
    ...
    result = runner.invoke(infra.app, ["logs", "--service", "graph-api", "--lines", "2"])
...
def test_logs_follow(monkeypatch, tmp_path):
    ...
    result = runner.invoke(infra.app, ["logs", "--service", "search-api", "-f", "--lines", "1"])
    assert result.exit_code == 0
...
def test_logs_docker(monkeypatch):
    ...
    result = runner.invoke(infra.app, ["logs", "--service", "neo4j", "--lines", "5", "-f"])
    assert result.exit_code == 0
