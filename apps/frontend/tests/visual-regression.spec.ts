    ...
    for (const theme of THEMES) {
      for (const viewport of VIEWPORTS) {
        test(`tab-components-${theme}-${viewport.name}`, async ({ page }) => {
          ...
          const tabVariants = ["default-tabs", "underline-tabs", "pills-tabs", "card-tabs"];
  ...
  test.describe("Cross-browser Visual Tests", () => {
    ["chromium", "firefox", "webkit"].forEach((browserName) => {
      test(`critical-components-${browserName}`, async ({ page }) => {
        ...
        const criticalComponents = ["tab-navigation", "form-inputs", "loading-spinners", "panels"];
...
test.describe("Tab Navigation Edge Cases", () => {
  ...
  test("tab-loading-states", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tab-loading-skeleton"]')).toHaveScreenshot(
      "tab-loading-state.png",
    );
  });
});
  ...
  test("high-contrast-mode", async ({ page }) => {
    ...
    for (const selector of criticalElements) {
      ...
      await expect(element).toHaveScreenshot(
        `high-contrast-${selector.replace(/[\[\]"=]/g, "")}.png`,
      );
    }
  });
});
...
test.describe("Performance Impact Visual Tests", () => {
  test("large-dataset-rendering", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="graph-container"]')).toHaveScreenshot(
      "large-dataset-graph.png",
    );
  });
  ...
  test("animation-performance", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tab-container"]')).toHaveScreenshot(
      "tab-animation-mid.png",
    );

    await page.waitForTimeout(200); // Animation complete
    await expect(page.locator('[data-testid="tab-container"]')).toHaveScreenshot(
      "tab-animation-end.png",
    );
  });
});
