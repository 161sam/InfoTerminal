...
export default function EthicsPage() {
  ...
  const run = async () => {
    const r = await fetch("/api/xai/explain", {
      method: "POST",
      ...
      <div className="max-w-5xl mx-auto space-y-6 p-4">
        <Panel title="Explain Text">
          <div className="space-y-2">
            ...
            />
            <button className="px-4 py-2 bg-primary-600 text-white rounded" onClick={run}>
              Explain
            </button>
          </div>
          {explain && (
            <div className="mt-4">
              ...
              <div className="mt-2 p-2 border rounded">
                {(explain.tokens || []).map((t: string, i: number) => {
                  ...
                  return (
                    <span key={i} className={matched ? "bg-yellow-200 mx-0.5" : "mx-0.5"}>
                      {t}
                    </span>
                  );
                })}
