...
const UserManagementPanel: React.FC<UserManagementPanelProps> = ({
  ...
  const roles = (user?.roles && user.roles.length > 0 ? user.roles : FALLBACK_ROLES).map((role) => role || 'User');
  ...
  return (
    ...
    >
      <Panel
        ...
        subtitle={showHeader ? 'Review profile details and platform access for the current session' : undefined}
        footer={(
          <div className="flex flex-wrap gap-2">
            <Button variant="primary" disabled title="Profile editing will be wired once the account API is available">
              <Settings size={16} className="mr-2" />
              Edit profile
            </Button>
            <Button variant="outline" disabled title="Password updates will be enabled once identity endpoints exist">
        ...
        <Panel.Body className={cn('space-y-6', !showHeader && 'mt-1')}>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            ...
            <div className="space-y-2">
              <div>
                <p className="text-lg font-semibold text-gray-900 dark:text-slate-100">{displayName}</p>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {roles.map((role) => (
                  <Badge key={role} className="bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-200">
                    {role}
                  </Badge>
                ))}
              </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/50">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">Security posture</p>
                  <p className="text-sm text-gray-600 dark:text-slate-400">
                    Multi-factor authentication and device approvals are pending backend integration.
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="outline" className="border-amber-300 text-amber-700 dark:border-amber-600 dark:text-amber-300">
                      MFA not configured
                    </Badge>
                    <Badge variant="outline" className="border-gray-300 text-gray-600 dark:border-gray-700 dark:text-slate-300">
            ...
            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/50">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">Roles & permissions</p>
                  <p className="text-sm text-gray-600 dark:text-slate-400">
                    Permissions reflect current policy; additional scopes will appear once connected to the identity service.
                  </p>
                  <div className="flex flex-wrap gap-2">
          ...
          <div className="rounded-xl border border-dashed border-gray-300 p-4 dark:border-gray-700">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">Session insights</p>
                <p className="text-sm text-gray-600 dark:text-slate-400">
                  Track recent sign-ins, active devices, and revoke tokens when backend telemetry becomes available.
                </p>
              </div>
            </div>
          </div>
