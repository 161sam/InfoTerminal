              ...
              <div key={col} className="border rounded p-2" onDragOver={onDragOver} onDrop={(e) => onDrop(e, col)}>
                ...
                <div className="space-y-2 min-h-[200px]">
                  {tasks.filter(t => t.status === col).map(t => (
                    <div key={t.id} className={`p-2 border-l-4 rounded bg-white ${prioColor(t.priority)} shadow-sm`} draggable onDragStart={(e) => onDragStart(e, t.id)}>
                      ...
                      <div className="mt-1 flex gap-1">
                        ...
                        <button className="text-xs px-2 py-1 bg-gray-100 rounded" onClick={() => {
                          const lb = prompt('Add label:')?.trim();
                          if (!lb) return;
                          const labels = [...(t.labels||[]), lb];
