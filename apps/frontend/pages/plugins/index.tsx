      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getHealthBadgeClass(health || "unknown")}`}
            >
        ...
        {plugin.category && (
          <div className="flex flex-wrap gap-2">
            ...
            {plugin.capabilities?.permissions?.slice(0, 2).map((perm, index) => (
              <span
                key={index}
                className="px-2 py-1 text-xs bg-gray-100 text-gray-600 rounded-full dark:bg-gray-800 dark:text-slate-400"
              >
                {perm}
              </span>
            ))}
...
export default function PluginsPage() {
  ...
  const openConfig = async (plugin: PluginItem, scope: "user" | "global") => {
    ...
    try {
      ...
      // Update local state
      setItems((prev) => prev.map((p) => (p.name === plugin.name ? { ...p, config } : p)));
    } catch {
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Extend your platform with powerful integrations"
    >
