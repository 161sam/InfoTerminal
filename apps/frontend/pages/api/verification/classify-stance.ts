...
export default async function handler(
  ...
  try {
    const {
      ...
      evidence,
      context
    }: ClassifyStanceRequest = req.body;
...
function generateMockStance(claim: string, evidence: string): StanceResponse {
  ...
  if (evidenceLower.includes('data') || evidenceLower.includes('study') || evidenceLower.includes('research')) {
    evidenceType = 'direct';
  ...
  if (stance === 'support') {
    ...
    reasoning += ' The evidence discusses the topic but does not clearly support or contradict the claim.';
  } else {
