...
function readState(): any {
  try { return JSON.parse(fs.readFileSync(STATE_FILE, 'utf8')); } catch { return { items: [] }; }
}
function writeState(data: any) {
  try {
    fs.mkdirSync(path.dirname(STATE_FILE), { recursive: true });
    fs.writeFileSync(STATE_FILE, JSON.stringify(data, null, 2));
  } catch { /* ignore in read-only env */ }
}
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!config || typeof config !== 'object') return res.status(400).json({ error: 'config must be object' });
