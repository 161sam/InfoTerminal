...
async function checkIncognitoMode() {
  try {
    ...
    const response = await fetchWithTimeout(
      `${opsControllerUrl}/api/security/incognito/status`,
      5000,
    );
...
async function checkDataProtection() {
  try {
    const opsControllerUrl = process.env.OPS_CONTROLLER_URL || "http://localhost:8618";
    const response = await fetchWithTimeout(
      `${opsControllerUrl}/api/security/containers/status`,
      5000,
    );
...
export default async function handler(
  ...
  try {
    ...
    const status: SecurityStatus = {
      ...
      incognitoMode:
        incognitoStatus.status === "fulfilled"
          ? incognitoStatus.value
          : {
              active: false,
            },
      dataProtection:
        dataProtectionStatus.status === "fulfilled"
          ? dataProtectionStatus.value
          : {
              ephemeralContainers: 0,
              memoryOnlyMode: false,
              autoWipeEnabled: false,
