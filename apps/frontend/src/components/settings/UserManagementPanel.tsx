  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-slate-100">Not authenticated</h3>
        <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">Please sign in to view user management.</p>
      </div>
    );
  }

  const displayName = user?.name?.trim() || user?.display_name?.trim() || 
    `${user?.first_name || ''} ${user?.last_name || ''}`.trim() || 
    user?.email?.split('@')[0] || 'User';
  
  const email = user?.email || 'user@example.com';
  const roles = (user?.roles && user.roles.length > 0 ? user.roles : FALLBACK_ROLES)
    .map((role) => role || 'User');
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 dark:bg-gray-900 dark:border-gray-800">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 space-y-3">
            <div>
              <h4 className="text-lg font-semibold text-gray-900 dark:text-slate-100">{displayName}</h4>
            ...
            <div className="flex flex-wrap gap-2">
              {roles.map((role, index) => (
                <Badge
                  key={index}
                  className={cn('text-xs font-medium', getRoleColor(role))}
                >
      ...
      {user?.sessionsCount !== undefined && (
        <div className="bg-white rounded-lg border border-gray-200 p-6 dark:bg-gray-900 dark:border-gray-800">
          <h5 className="mb-3 font-medium text-gray-900 dark:text-slate-100">Session Information</h5>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-gray-500 dark:text-slate-400">Active Sessions:</span>
              <span className="ml-2 font-medium text-gray-900 dark:text-slate-100">{user.sessionsCount}</span>
            </div>
            <div>
