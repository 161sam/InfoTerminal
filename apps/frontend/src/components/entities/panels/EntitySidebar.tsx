...
import { Entity, EntityFilter, ENTITY_TYPE_ICONS, RISK_LEVELS } from "@/lib/entities/entity-config";
...
export function EntitySidebar({ entities, filters, onFilterChange }: EntitySidebarProps) {
  ...
  const typeDistribution = entities.reduce(
    (acc, entity) => {
      ...
      <Panel title="Risk Levels">
        <div className="space-y-3">
          {riskDistribution.map((level) => {
            const percentage =
              getTotalEntities() > 0 ? (level.count / getTotalEntities()) * 100 : 0;
            ...
            return (
              <button
                ...
                onClick={() =>
                  onFilterChange(
                    "riskLevel",
                    level.value === filters.riskLevel ? "all" : level.value,
                  )
                }
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-500 dark:text-slate-400">{level.count}</span>
                </div>
