        ...
        if (e.type === "entity") {
          return (
            <span key={`entity:${e.value}`}>
              entity: {e.value} <button onClick={() => onRemoveEntity?.(e.value)}>x</button>
            </span>
          );
        }
        if (e.type === "value") {
          return (
            <span key={`value:${e.value}`}>
              value: {e.value} <button onClick={() => onRemoveValue?.(e.value)}>x</button>
            </span>
          );
        }
        return (
          <span key={`${e.facet}:${e.value}`}>
            {e.facet}: {e.value} <button onClick={() => onRemove(e.facet!, e.value)}>x</button>
          </span>
        );
      })}
