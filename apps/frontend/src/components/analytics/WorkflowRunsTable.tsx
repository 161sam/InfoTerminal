          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {["running", "completed", "failed", "cancelled"].map((status) => {
              ...
              return (
                <div
                  key={status}
                  className={`p-3 rounded-lg ${getStatusColor(status as WorkflowRun["status"])}`}
                >
                    ...
                    >
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`inline-block px-2 py-1 text-xs rounded ${getStatusColor(run.status)}`}
                          >
                            {run.status}
                          </span>
                        </div>
                          ...
                          <div className="space-y-1">
                            ...
                            {run.output.confidence && (
                              <div className="text-xs">
                                <span
                                  className={`${
                                    run.output.confidence >= 0.8
                                      ? "text-green-600 dark:text-green-400"
                                      : run.output.confidence >= 0.6
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div>
                  <div className="text-lg font-bold text-gray-900 dark:text-gray-100">
                    {Math.round(
                      data.reduce((sum, run) => sum + (run.duration || 0), 0) / data.length / 1000,
                    )}
                    s
                  </div>
                ...
                <div>
                  <div className="text-lg font-bold text-green-600 dark:text-green-400">
                    {Math.round(
                      (data.filter((run) => run.status === "completed").length / data.length) * 100,
                    )}
                    %
                  </div>
