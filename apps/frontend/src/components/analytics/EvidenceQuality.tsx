...
export function EvidenceQuality({ filters, className = "" }: EvidenceQualityProps) {
      ...
      >
        <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
          ...
          <span className="text-sm">
            Evidence quality service unavailable. Showing empty state.
          </span>
        </div>
      </div>
    );
  ...
  const verificationRate =
    data && data.totalClaims > 0 ? Math.round((data.verifiedClaims / data.totalClaims) * 100) : 0;
            ...
            <div>
              ...
              <div className="space-y-3">
                {data.qualityMetrics.map((metric, index) => (
                  <div
                    key={index}
                    className={`border rounded-lg p-4 ${getQualityColor(metric.status)}`}
                  >
                  ...
                  <tbody>
                    {data.sourceReliability.slice(0, 8).map((source, index) => (
                      ...
                      >
                        ...
                        <td className="p-2 text-right font-mono">{source.claimsCount}</td>
                        <td className="p-2 text-right font-mono">
                          {Math.round(source.verificationRate * 100)}%
                        </td>
                        <td className="p-2 text-right">
                          <span
                            className={`font-medium ${getReliabilityColor(source.reliability)}`}
                          >
                ...
                {data.recommendations.map((rec, index) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <span
                        className={`inline-block px-2 py-1 text-xs rounded font-medium ${getPriorityColor(rec.priority)}`}
                      >
