    ...
    return (
      <div className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
          <Search size={20} />
          <span className="text-sm">Query insights service unavailable. Feature may not be enabled.</span>
        </div>
      </div>
    );
  }
                  ...
                  <div>
                    ...
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data.topQueries.slice(0, 10)} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-700">
                            <th className="text-left p-2 font-medium text-gray-900 dark:text-gray-100">Query</th>
                            <th className="text-right p-2 font-medium text-gray-900 dark:text-gray-100">Count</th>
                            <th className="text-right p-2 font-medium text-gray-900 dark:text-gray-100">Avg Results</th>
                            <th className="text-right p-2 font-medium text-gray-900 dark:text-gray-100">CTR</th>
                            <th className="text-left p-2 font-medium text-gray-900 dark:text-gray-100">Last Used</th>
                          </tr>
                        </thead>
                        <tbody>
                          {data.topQueries.slice(0, 15).map((query, index) => (
                            <tr key={index} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/50">
                              ...
                              <td className="p-2 text-right font-mono">
                                {query.averageResults}
                              </td>
                              <td className="p-2 text-right">
                                <span className={`font-medium ${
                                  query.clickthrough >= 0.5 ? 'text-green-600 dark:text-green-400' :
                                  query.clickthrough >= 0.2 ? 'text-yellow-600 dark:text-yellow-400' :
                      ...
                      {data.searchPatterns.map((pattern, index) => (
                        <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <div
                                ...
                                style={{ backgroundColor: getPatternCategoryColor(pattern.category) }}
                              />
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`text-xs font-medium ${getEffectivenessColor(pattern.effectiveness)}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(data.clickthrough.documentTypes).map(([type, count]) => (
                      <div key={type} className="text-center p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-200 dark:border-gray-700">
                            <th className="text-left p-2 font-medium text-gray-900 dark:text-gray-100">Query</th>
                            <th className="text-right p-2 font-medium text-gray-900 dark:text-gray-100">Response Time</th>
                            <th className="text-left p-2 font-medium text-gray-900 dark:text-gray-100">Timestamp</th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr key={index} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-900/50">
                              ...
                              <td className="p-2 text-right">
                                <span className={`font-mono ${
                                  slowQuery.responseTime > 5000 ? 'text-red-600 dark:text-red-400' :
                                  slowQuery.responseTime > 2000 ? 'text-yellow-600 dark:text-yellow-400' :
