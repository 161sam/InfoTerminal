  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Dossier Items</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {getItemsByType().map(({ type, count, label }) => (
          <div key={type} className={`p-3 rounded-lg border text-center ${getItemTypeColor(type)}`}>
        ...
        {isEditing ? (
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500 mt-1">
              {item.metadata?.lastSeen && <span>Last seen: {item.metadata.lastSeen}</span>}
