...
export default function ModernGraphViewer() {
  ...
  const expandGraph = async (nodeId: string, limit: number = 50) => {
    setLoading(true);
    try {
      const response = await fetch(`http://127.0.0.1:8002/neighbors?node_id=${encodeURIComponent(nodeId)}&limit=${limit}`);
  ...
  const saveView = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8004/views', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          name: viewName,
          nodes: elements.filter(el => !el.data.source).map(el => el.data),
          edges: elements.filter(el => el.data.source).map(el => el.data),
          positions
        })
      });
  ...
  const loadView = async (viewId: number) => {
    try {
      const response = await fetch(`http://127.0.0.1:8004/views/${viewId}`, {
        headers: { 'X-User': 'dev' }
      });
  ...
  const filteredElements = useMemo(() => {
    ...
    return elements.filter(el => {
      if (el.data.source) return true; // Always show edges
  ...
  return (
    <DashboardLayout 
      ...
      subtitle="Interactive network visualization and analysis"
    >
