  ...
  const search = async () => {
    ...
    try {
      if (hybrid) {
        ...
        const r = await fetch(`/api/rag/law/retrieve?q=${encodeURIComponent(query)}&rerank=${rerank ? '1' : '0'}`);
        const data = await r.json();
        setResults(data.items || []);
      }
    } finally { 
          ...
          <div className="space-y-3">
            {results.map((item, idx) => (
              <div key={idx} className={`${cardStyles.base} ${cardStyles.padding} space-y-2`}>
                ...
                <div className={`${textStyles.body} whitespace-pre-wrap`}>
                  {item.text}
                </div>
              </div>
            ))}
      ...
      <Panel title="Legal Context by Entity">
        <div className="space-y-4">
          <div className="flex gap-2 items-center">
            ...
            />
            <button 
              className={compose.button('secondary', loading ? 'opacity-50 cursor-not-allowed' : '')}
              onClick={loadContext} 
              disabled={loading}
            >
              {loading ? <LoadingSpinner size="sm" variant="secondary" layout="inline" /> : 'Load Context'}
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {!context.length && (
              <div className={textStyles.body}>No legal context found</div>
            )}
          </div>
        </div>
      </Panel>
