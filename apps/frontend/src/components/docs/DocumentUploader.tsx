  ...
  return (
    <div>
      <div style={{ marginBottom: '1rem' }}>
        <button onClick={() => setTab('file')} disabled={tab === 'file'}>Datei hochladen</button>
        <button onClick={() => setTab('text')} disabled={tab === 'text'}>Text einfügen</button>
      </div>
      {tab === 'file' ? (
        <form onSubmit={handleFileSubmit}>
          <input type="file" name="file" accept=".pdf,.txt" required />
          <input type="text" name="title" placeholder="Titel" />
          <button type="submit" disabled={loading}>{loading ? 'Lade...' : 'Upload'}</button>
        </form>
      ) : (
        <form onSubmit={handleTextSubmit}>
          <input type="text" name="title" placeholder="Titel" />
          <textarea name="text" rows={6} required />
          <button type="submit" disabled={loading}>{loading ? 'Lade...' : 'Annotieren'}</button>
        </form>
      )}
    </div>
  );
