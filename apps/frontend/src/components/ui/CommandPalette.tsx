...
function CommandPalette({ customCommands }: { customCommands: Command[] }) {
  ...
  const builtInCommands: Command[] = useMemo(
    ...
    [router, close, theme, user, setTheme, logout, notifications],
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {command.subtitle && (
          <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{command.subtitle}</p>
        )}
      </div>
...
export function useQuickActions() {
  ...
  const addQuickAction = useCallback(
    (id: string, title: string, action: () => void, options?: Partial<Command>) => {
      addCommand({
        id: `quick-${id}`,
        title,
