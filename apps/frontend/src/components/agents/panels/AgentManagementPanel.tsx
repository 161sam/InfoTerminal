        ...
        <div className={`${cardStyles.base} ${cardStyles.padding}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${textStyles.h2}`}>{healthyServices}/{totalServices}</p>
            </div>
        ...
        <div className={`${cardStyles.base} ${cardStyles.padding}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${textStyles.h2}`}>{multiAgentEnabled ? 'ON' : 'OFF'}</p>
            </div>
        ...
        <div className={`${cardStyles.base} ${cardStyles.padding}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-2xl font-bold ${textStyles.h2}`}>{workflowsEnabled ? 'Active' : 'Inactive'}</p>
            </div>
            <div className={`p-3 rounded-full ${
              workflowsEnabled 
      ...
      <Panel title="Agent Capabilities">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCapabilities.map((capability) => (
            <div key={capability.id} className={`${cardStyles.base} p-4 hover:shadow-lg transition-shadow`}>
              <div className="flex items-start justify-between mb-3">
                <div className={`p-2 rounded-lg ${capability.color.replace('text-', 'bg-').replace('600', '100')} dark:bg-opacity-20`}>
                  <capability.icon size={20} className={capability.color} />
                </div>
                <span className={compose.status('info')}>
                  {capability.expertise.length} skills
                </span>
              </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {capability.tools.slice(0, 3).map((tool, idx) => (
                      <span key={idx} className={`${textStyles.bodySmall} px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded`}>
                        {tool}
                      </span>
                    ))}
                    {capability.tools.length > 3 && (
                      <span className={`${textStyles.bodySmall} px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {capability.expertise.slice(0, 2).map((skill, idx) => (
                      <span key={idx} className={`${textStyles.bodySmall} px-2 py-1 bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300 rounded`}>
                        {skill}
                      </span>
                    ))}
                    {capability.expertise.length > 2 && (
                      <span className={`${textStyles.bodySmall} px-2 py-1 bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300 rounded`}>
            ...
            <div className="space-y-2">
              {Object.entries(apis).map(([name, url]) => (
                <div key={name} className="flex items-center justify-between">
                  ...
                  <span className={`${textStyles.bodySmall} font-mono truncate max-w-40`}>{url}</span>
                </div>
              ))}
            </div>
          </div>
