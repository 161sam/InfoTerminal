...
export default function useFileUpload(
  url: string = "/api/documents/upload",
  ...
  const uploadSingle = useCallback(
    async (item: UploadItem) => {
      ...
      setUploads((prev) =>
        prev.map((upload) =>
          upload.id === item.id
            ? { ...upload, status: "uploading", progress: 0, message: undefined }
            : upload,
        ),
      );
      ...
      try {
        ...
        xhr.upload.addEventListener("progress", (event: ProgressEvent) => {
          if (event.lengthComputable) {
            ...
            setUploads((prev) =>
              prev.map((upload) => (upload.id === item.id ? { ...upload, progress } : upload)),
            );
          }
        });
  ...
  const cancelUpload = useCallback(
    (id: string) => {
      ...
      setUploads((prev) =>
        prev.map((upload) =>
          upload.id === id
            ? { ...upload, status: "cancelled", message: "Upload cancelled" }
            : upload,
        ),
      );
    },
